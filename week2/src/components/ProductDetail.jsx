import React from 'react'
import styled from 'styled-components'
import '../css/ProductDetail.css'
import {useState, useEffect} from 'react';

const TitlePurple = styled.div`
    font-size: 12px;
    color: rgb(95, 0, 128);
    line-height: 19px;
    height: 19px;
    letter-spacing: -0.5px;
    margin-top: 14px;
`

const First = styled.div`
    display: flex;
    flex-direction: row;
    border-top: 1px solid rgb(244, 244, 244);
    font-size: 13px; 
    letter-spacing: -0.5px;
    align-items: flex-start;
    padding: 17px 0px 18px;
    width: 100%;
    border-bottom: 1px solid rgb(244, 244, 244);
`

const FirstTitle = styled.div`
    color: rgb(102, 102, 102);
`

const Definition = styled.div`
    margin-top: 20px;
`

const SubDetail = styled.div`
    display: block;
    font-size: 10px;
    color: rgb(102, 102, 102);
    padding-top: 4px;
    line-height: 12px;
    white-space: pre-line;
`

const Letter = styled.p`
    color: #666;
    padding-top: 3px;
`

const TextGuide = styled.span`
    font-weight: 550;
`

const ProductDetail = () => {
    const price = 123300;

    const [count, setCount] = useState(0);
    const [total, setTotal] = useState(0);

    const incrementCount = () => {
        setCount(count+1);
    }

    const decrementCount = () => {
        if(count > 0){
            setCount(count-1);
        }
    } 

    useEffect(() => { //마운팅 - count가 변화할 때만 setTotal 함수 실행됨
        const totalPrice = price * count;
        setTotal(totalPrice.toLocaleString());
        console.log('count 값 변화');
    },[count])

    return (
    <div>
    <div className="detail">
        <div className="detail-container">
            <img src="https://product-image.kurly.com/product/image/eae57e3b-1c19-4d22-9962-acfc09c7b640.jpg"></img>
            <div className="detail-right">
                <span>샛별배송</span>
                <div className="detail-title">
                    <h1>[산타마리아노벨라] 프리지아 오드코롱 2종 (택1)</h1>
                    <img src="data:image/svg+xml;base64,"></img>
                </div>
                <p className="clean">깨끗하고 청초한 프리지아의 향기</p>
                <div className="color">
                    <span className="percent">10%</span>
                    <span className="money">123,300</span>
                    <span className="won">원~</span>
                </div>
                <p className="original-won">137,000원<img src="data:image/svg+xml;base64,"></img></p>
                <TitlePurple>로그인 후, 적립 혜택이 제공됩니다.</TitlePurple>
                <button className="coupon-btn"><span>산타마리아노벨라 쿠폰 보러가기</span><img src="data:image/svg+xml;base64,"></img></button>
                <Definition>
                    <First>
                        <FirstTitle>배송</FirstTitle>
                        <dd className="first-detail1">
                            <pre>샛별배송</pre>
                            <SubDetail>23시 전 주문 시 내일 아침 7시 전 도착</SubDetail>
                            <SubDetail>(대구·부산·울산 샛별배송 운영시간 별도 확인)</SubDetail>
                        </dd>
                    </First>
                    <First>
                        <FirstTitle>판매자</FirstTitle>
                        <dd class="first-detail2">
                            <pre>컬리</pre>
                        </dd>
                    </First>
                    <First>
                        <FirstTitle>포장타입</FirstTitle>
                        <dd class="first-detail3">
                            <pre>상온 (종이포장)</pre>
                            <SubDetail>택배배송은 에코 포장이 스티로폼으로 대체됩니다.</SubDetail>
                        </dd>
                    </First>
                    <First>
                        <FirstTitle>안내사항</FirstTitle>
                        <dd class="first-detail3">
                            <pre>이드랄리아 리추얼 트라이얼 키트와 토탈 프래그런스 키트</pre>
                            <pre>증정품 소진 완료되어 이벤트 종료되었습니다. 구매 전 참고 부탁드립니다.</pre>
                        </dd>
                    </First>
                    <First>
                        <FirstTitle>상품 선택</FirstTitle>
                        <dd className="first-detail-box">
                            <div className="count">
                                <button id="decrement" onClick={decrementCount}></button>
                                <span id="num-count1">{count}</span>
                                <button id="increment" onClick={incrementCount}></button>
                            </div>
                        </dd>
                    </First>

                    <div class="detail-right-bottom">
                        <div class="cartwon">
                            <span class="letter1">총 상품금액: </span>
                            <span class="letter2">{total}</span>
                            <span class='won'>원</span>
                        </div>
                        <div class="cartwon-detail">
                            <button>적립</button>
                            <span>로그인 후, 적립 혜택 제공</span>
                        </div>
                        <div class="img-flex">
                            <button class="emo"><img src="data:image/svg+xml;base64,"></img></button>
                            <button class="emo"><img src="data:image/svg+xml;base64,"></img></button>
                            <button class="purple">장바구니 담기</button>
                        </div>
                    </div>
                </Definition>
            </div>
        </div>
        <nav class="detail-menu">
            <ul>
                <li>상품설명</li>
                <li>상세정보</li>
                <li>후기(17)</li>
                <li>문의</li>
            </ul>
        </nav>
        <div class="context">
            <img src="https://img-cf.kurly.com/shop/data/goodsview/20230125/gv40000479681_1.jpg" className='detail-img1'></img>
            <img src-="https://img-cf.kurly.com/shop/data/goodsview/20230125/gv40000479682_1.jpg" className='detail-img2'></img>
            <img src="https://img-cf.kurly.com/shop/data/goodsview/20230125/gv20000479683_1.jpg" className='detail-img3'></img>
            <div>
                <h2 className='text-heritage'>800년의 헤리티지를 잇는 새로운 이야기,</h2>
                <h2 className='text-edition'>피렌체 1221 에디션</h2>
                <Letter>산타마리아노벨라의 800년 헤리티지를 현대적으로 재해석한 피렌체 1221 에디션을 소개합니다. 산타마리아노벨라의 아이코닉한 향이 담</Letter>
                <Letter>긴 오 드 코롱과 바디 컬렉션, 왁스 타블렛, 세 가지의 에디션을 새롭게 선보였어요. 오래전 수도사들이 사용했던 유리병을 오마쥬한 투명한</Letter>
                <Letter>향수 바틀, 산타마리아노벨라의 문양이 새겨진 골드 캡, 그리고 피렌체의 전통 양식을 계승한 라벨 디자인까지. 새로운 패키지로 태어난 산타</Letter>
                <Letter>마리아노벨라의 특별한 에디션을 만나보세요.</Letter>
            </div>
            <img src="https://img-cf.kurly.com/shop/data/goodsview/20230125/gv10000479684_1.jpg" className='kurlypick-img'></img>
            <div class="text">
                <h2 className='text-freejia'>01~02. 프리지아 오 드 코롱 2종</h2>
                <div className='soap-wrapper'>
                    <TextGuide>・ 선택 가이드</TextGuide><span>: ①50mL, ②100mL 중 선택하세요.</span><br/>
                    <TextGuide>・ 특징</TextGuide><span>: 깨끗한 비누처럼 포근하고 섬세한 프리지아 향이에요.</span>
                </div>
            </div>
            <img src="https://img-cf.kurly.com/shop/data/goodsview/20230126/gv10000479685_1.jpg" className='kurlytip-img'></img>
            <div class="text">
                <h2 className='text-check'>구입 전 확인하세요</h2>
                <div className='text-shoppingbag'>
                    <span>・ 제품을 넣을 수 있는 전용 쇼핑백을 함께 보내드립니다.</span>
                    
                </div>
            </div>
            <img src="https://img-cf.kurly.com/shop/data/goodsview/20230125/gv30000479687_1.jpg" className='aboutbrand-img'></img>
            <div class='text-santamariano'>
                <Letter>산타마리아노벨라는 1221년 이탈리아 문화의 중심, 피렌체에서 시작된 라이프스타일 뷰티 브랜드입니다. 수도사들이 수도원의 정원에서</Letter>
                <Letter>기른 허브와 약초로 연고와 밤 등을 만들던 것에서 그 역사가 시작되었죠. 1332년 수도사들이 허브 추출물로 한 상인을 치료하며 그 효능이</Letter>
                <Letter>수도원 밖으로 처음 알려지게 되는데요. 이후 흑사병으로 고통받는 사람들을 위해 '아쿠아 디 로즈'를 제조하는 등 수 세기에 걸쳐 다양한 제</Letter>
                <Letter>품을 개발하죠. 1612년 산타마리아노벨라는 자신들의 약국을 대중들에게 공식적으로 개방했고, 주변 국가의 왕실과 저명한 예술가들에게</Letter>
                <Letter>사랑을 받으며 그 명성을 널리 알렸습니다. 현재까지도 피렌체의 정원에서 얻은 재료와 전통 그대로의 방식을 고수하며 제품을 생산하고 있</Letter>
                <Letter>어요. 800년간 이어져온 산타마리아노벨라의 오랜 역사와 이야기를 만나보세요.</Letter>
            </div>
            <img src="https://img-cf.kurly.com/shop/data/goodsview/20230125/gv00000479686_1.jpg" className='adver-img'></img>
            <img src="https://img-cf.kurly.com/shop/data/goodsview/20230125/gv20000479688_1.jpg"></img>
        </div>        
    </div>
    </div>
  )
}

export default ProductDetail